...
const A1VocabularySection = ({
  ...
  const quizData = currentA1VocabData?.quiz?.[difficulty] || [];
  const fillData = currentA1VocabData?.fillBlanks?.[difficulty] || [];
    ...
    <div>
      ...
      <div className="bg-white rounded-lg shadow-lg p-4 mb-4">
        <div className="flex gap-2 flex-wrap mb-4">
          {a1Vocabulary.map((item, index) => (
            <button
              key={index}
              onClick={() => {
                setCurrentA1Vocab(index);
                handleReset();
              }}
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                index === currentA1Vocab
      ...
      {mode === "learn" && (
        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-5xl">{currentA1VocabData.icon}</span>
            <div>
              <h2 className="text-3xl font-bold text-sky-600">
                {currentA1VocabData.name}
              </h2>
              <p className="text-xl text-gray-600">
                {currentA1VocabData.polish}
              </p>
            </div>
          </div>

          {currentA1VocabData.description && (
            <div className="mb-6 p-4 bg-sky-50 rounded-lg">
              <h3 className="font-bold text-gray-800 mb-2">📚 Description:</h3>
              <p className="text-gray-700">{currentA1VocabData.description}</p>
            </div>
          )}
      ...
      {mode === "quiz" && (
        <div className="bg-white rounded-lg shadow-lg p-8">
          {quizData.length === 0 ? (
            ...
            <div>
              <h2 className="text-2xl font-bold text-sky-600 mb-6">Quiz</h2>
                            ...
                            <button
                              ...
                              className={`w-full text-left p-3 rounded-lg border-2 transition-colors ${
                                quizAnswers[qIndex] === oIndex
                                  ...
                                  : "bg-gray-50 border-gray-300 hover:border-sky-300"
              ...
              {showResults ? (
                ...
                <div className="mt-6 flex gap-3">
                  <button
                    ...
                    className="flex-1 bg-sky-600 text-white py-3 rounded-lg font-bold hover:bg-sky-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors">
                    Submit Quiz
                  </button>
                </div>
              )}
...
export default A1VocabularySection;
